...
func AddProductToShoppingCart(userId int, product models.Product) error {
	...
	_, err = DB.Exec("insert into positions_ordered (order_id, price, article, supplier, supplier_price_num, brand, currency, currency_rate, delivery, weight, quantity, product_quantity_price) values (?,?,?,?,?,?,?,?,?,?,?,?)",
...
func GetProductsUnderNonConfirmedOrderId(userId int, orderId string) ([]*models.Product, error) {
